<template>
    <section :class="['w-full', containerClass]">
        <section :class="['w-full h-24 relative', sectionClass]">
            <textarea @input="onInput" v-model="value" :type="props.type"
                :class="['peer w-full h-full pt-4 pb-2 resize-none bg-white outline outline-0 focus:outline-0 disabled:border-0 border-2 focus:border-2 text-sm px-3 py-2.5 rounded-[7px] border-gray-900 placeholder-shown:border-0 placeholder-shown:bg-gray-200 focus:bg-white', inputClass]"
            ...
            <button @click="clearInput" type="button"
                :class="['absolute right-2 top-6 transform -translate-y-1/2 peer-placeholder-shown:invisible', buttonClass]">
